...
def _provenance():
    """Which build produced this probe -- branch, commit, dirty flag.

    Two probes are only comparable if it is recorded WHAT each one ran against. Flash coarse
    tiling measured fine on 2026-07-30 and fails now, so the interesting question is always
    "which tree", and a bare pass/fail table cannot answer it.
    """

    def git(*a):
        try:
            return subprocess.run(
                ["git", *a], cwd=_ROOT, capture_output=True, text=True, timeout=10
            ).stdout.strip()
...
def _delta(before, tag, us):
    """FIXED / STILL BROKEN / REGRESSED, against the previous probe of the same config."""
    if not before:
        return ""
    was_ok, now_ok = bool(before.get("kernel_us")), bool(us)
    if now_ok and not was_ok:
        return "   <- FIXED"
    if was_ok and not now_ok:
        return "   <- REGRESSED"
    if not now_ok and before.get("result") != tag:
        return f"   (was {before.get('result')})"
    return ""
...
def main():
    ...
    if os.path.exists(args.out):
        try:
            with open(args.out, encoding="utf-8") as fh:
                old = json.load(fh)
            rows_old = old.get("rows", old) if isinstance(old, dict) else old
            prior = {r["name"]: r for r in rows_old}
            oldprov = old.get("provenance", {}) if isinstance(old, dict) else {}
            if oldprov:
                print(
                    f"comparing against {oldprov.get('branch')}@{oldprov.get('commit')}"
                    f"{' (dirty)' if oldprov.get('dirty') else ''}"
                )
        except Exception:  # noqa: BLE001
            prior = {}
    print(
        f"probing on {prov['branch']}@{prov['commit']}"
        f"{' (dirty)' if prov['dirty'] else ''}: {prov['subject'][:60]}"
    )
    print(f"{len(cases)} flash configuration(s), 1 warmup + 1 rep each\n")
    print(
    ...
    for c in cases:
        ...
        print(
            ...
            f"{_delta(prior.get(c['name']), tag, us)}"
        )
        with open(args.out, "w", encoding="utf-8") as fh:
            json.dump({"provenance": prov, "rows": rows}, fh, indent=2)
