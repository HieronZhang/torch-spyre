...
```bash
python3 docs/source/user_guide/examples/run_cost_model_sweep.py   # needs hardware, a few hours
```

That re-measures every configuration and writes `sweep_records.json` into
`tools/cost_model/`. It is not committed: ~14 MB of JSON that grows with every re-sweep would
dominate the history for a file no diff can usefully show.
...
it automatically; treat it as a starting point, not as a measurement of your build.
