...
#
#   bash docs/source/user_guide/examples/run_outlier_sweep.sh                 # all
...
has NOISE0 && { sect NOISE0 "control + flagged noisy points, x3 invocations, 10 reps each"
  for rep in 1 2 3; do
...
has MMCORE && { sect MMCORE "matmul forced to cores {1,2,4,8,16,32} across K-ratios + shapes"
  for KK in 512 1024 2048 4096; do                 # M=N=2048, vary K (compute/mem ratio)
    for s in "1 1 1" "1 2 1" "2 2 1" "2 4 1" "4 4 1" "4 8 1"; do runmmwd 2048 "$KK" 2048 $s; done
  done
  for s in "1 1" "1 2" "2 2" "2 4" "4 4" "4 8"; do runmmwd 1024 2048 1024 $s 1; done  # small sq
  for s in "1 2" "2 2" "2 4" "4 4" "4 8"; do runmmwd 4096 2048 2048 $s 1; done  # M=4096
  for s in "1 2" "2 2" "2 4" "4 4" "4 8"; do runmmwd 2048 2048 4096 $s 1; done  # N=4096
  esect; }

# ============ BMMLAY (cat 4): device layout [1,0,2] vs [0,1,2], copy-checked ==
has BMMLAY && { sect BMMLAY "full bmm, {[0,1,2],[1,0,2]}^2 operand layouts, B in {2,4,8,16}, shapes"
  for B in 2 4 8 16; do
    for LL in "0,1,2 0,1,2" "1,0,2 0,1,2" "0,1,2 1,0,2" "1,0,2 1,0,2"; do
      runbmml "$B" 1024 2048 1024 1 4 8 1 $LL
    done
  done
  for LL in "0,1,2 0,1,2" "1,0,2 0,1,2" "1,0,2 1,0,2"; do
    runbmml 4 2048 2048 1024 1 4 8 1 $LL   # 2nd shape (bigger M)
    runbmml 8 1024 2048 2048 1 4 8 1 $LL   # 3rd shape (bigger N)
  done
  esect; }

# ============ XPORT (cat 2): BW_eff(R,C) surface for the 4 transport ops ======
has XPORT && { sect XPORT "transpose/transpose_outer/cat0/cat1: fix-C sweep-R, fix-R sweep-C, diag"
...
# ============ BCAST (cat 1): write + broadcast ops, large-C focus ============
has BCAST && { sect BCAST "write full grid + bcast/mulbcast/bcastcol/copy dense large-C"
  for R in 512 2048 8192 16384; do for C in 1024 4096 16384; do runpw write "$R" "$C" 0; done; done
  for op in bcast mulbcast bcastcol copy; do
    for R in 256 2048 8192; do for C in 2048 8192 16384; do runpw "$op" "$R" "$C" 0; done; done
  done
  esect; }

# ============ SMXUNROLL (cat 5): unrolled softmax + small-COLS, IR captured ===
has SMXUNROLL && { sect SMXUNROLL "softmax_unrolled shapes x tiles + softmax_row_tiling small-COLS"
  for sh in "1024 512" "2048 512" "2048 2048" "4096 2048"; do
    for t in 1 4 8 16; do runsmx softmax_unrolled $sh "$t"; done
  done
  for C in 128 256 512; do for t in 1 4 8; do runsmx softmax_row_tiling 2048 "$C" "$t"; done; done
  for C in 128 256;     do for t in 4 8;   do runsmx softmax_row_tiling 4096 "$C" "$t"; done; done
  esect; }

# ============ CTMM (cat 6): coarse matmul/bmm tile ladders + LX-spill =========
has CTMM && { sect CTMM "matmul_row/k/nested ladders (multi-shape) + coarse bmm + LX-spill"
  ...
  for t in 1 2 4 8 16; do runmk matmul_row_tiling 2048 2048 4096 "$t"; done
  for t in 1 2 4 8 16; do runmk matmul_k_tiling   2048 2048 2048 "$t"; done   # control
  for t in 1 2 4 8 16; do runmk matmul_k_tiling   1024 4096 1024 "$t"; done
  for t in 1 2 4 8;    do runmk mm_nested_m_k     2048 2048 2048 "$t"; done
  for t in 1 2 4 8;    do runmk mm_nested_m_k     4096 2048 2048 "$t"; done
  ...
  for t in 1 2 4;      do runmkb bmm_k_tiling      8 1024 2048 1024 "$t"; done
...
has BMM2R && { sect BMM2R "full bmm vs 3d2d (shared weight), balanced 4x8, sweep B/K/shape"
  for B in 2 4 8 16; do
  ...
  for sh in "4 2048 2048 1024" "4 1024 2048 2048"; do
...
# compile -> FLASH_TIMEOUT. Default shape H=32,Lq=Lk=4096,D=128; a 2048 shape for density.
# Runs LAST: the heaviest / most exploratory section, after every crux run has landed.
has FLASH && { sect FLASH "flash_attn: sweep Lq/H tile counts + work_div, 2 shapes, IR saved"
  ...
  # -- smaller shape (Lq=Lk=2048): denser, faster-compiling
