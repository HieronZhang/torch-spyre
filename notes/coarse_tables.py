...
in the write-up is ever hand-typed. Run after any model change:
    ...
    python3 notes/coarse_tables.py            # markdown to stdout
    python3 notes/coarse_tables.py --all      # include out-of-target rows too
...
def emit_table(rows, cap=28):
    ...
    if total > cap:  # thin evenly rather than truncating, so the range still shows
        ...
        print(
            ...
            "all of them: `python3 notes/coarse_tables.py`.\n"
        )
    for d in rows:
