...
coefficient changes.
    ...
    python3 notes/part_tables.py 1          # Part I   (pointwise)
    python3 notes/part_tables.py 2          # Part II  (other memory-bound ops)
    python3 notes/part_tables.py 3          # Part III (matmul)
...
def emit(part, rows, full=False):  # noqa: C901
    ...
    if not full:
        print(
            ...
            f"All {len(rows)} configurations: `python3 notes/part_tables.py {part} --full`."
        )
    rows = shown
    wide = part == 3
    hdr = (
